...
const ResponsiveAppBar = () => {
  ...
  const renderMobileMenu = () => (
    ...
    >
      {Object.keys(pages).map((menuKey) => {
        const menuItem = menuConfig.menuItems.find(item => item.key === menuKey);
        return (
          <React.Fragment key={menuKey}>
            ...
            <Collapse in={openSubMenuMobile[menuKey]} timeout="auto" unmountOnExit>
              <Box sx={{ pl: 2 }}>
        ...
        return (
          <Box key={menuKey} sx={{ position: 'relative' }}>
            <Button
              ...
              endIcon={openSubMenuDesktop[menuKey] ? <ExpandLess /> : <ExpandMore />}
            >
              {menuItem.label}
            </Button>
              ...
              {({ TransitionProps, placement }) => (
                ...
                >
                  <Paper elevation={3} sx={{ mt: 1 }}>
                    <ClickAwayListener onClickAway={() => handleCloseSubMenuDesktop(menuKey)}>
  ...
  const renderUserMenu = () => (
    ...
    >
      ...
      <MenuItem component={Link} to="/perfil" onClick={() => setAnchorElUser(null)} sx={{ gap: 1.5 }}>
        <AccountCircleIcon fontSize="small" /> Mi Perfil
      </MenuItem>

      {user.rol === 'admin' && (
        <MenuItem component={Link} to="/configuracion" onClick={() => setAnchorElUser(null)} sx={{ gap: 1.5 }}>
