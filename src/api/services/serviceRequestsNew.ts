...
export class ServiceRequestsService {
  ...
  async updateServiceRequest(
    ...
    const response = await api.put<ApiResponse<ServiceRequest>>(
      `${this.basePath}/${id}`,
      updateData
    );
    if (!response.data.data) {
      throw new Error(
        response.data.error || "Error al actualizar la solicitud"
      );
    }
    return response.data.data;
  }
  ...
  async searchServiceRequests(
    query: string,
    filters?: Omit<ServiceRequestFilters, "name">
  ): Promise<PaginatedResponse<ServiceRequest>> {
  ...
  async getServiceRequestsByStatus(
    status: "pendiente" | "en_progreso" | "completado" | "cancelado",
    filters?: Omit<ServiceRequestFilters, "status">
  ): Promise<PaginatedResponse<ServiceRequest>> {
   ...
   */
  async getServiceRequestsByDateRange(
    startDate: string,
    endDate: string,
    filters?: Omit<ServiceRequestFilters, "startDate" | "endDate">
  ): Promise<PaginatedResponse<ServiceRequest>> {
  ...
  async getServiceRequestsByType(
    serviceType: string,
    filters?: Omit<ServiceRequestFilters, "serviceType">
  ): Promise<PaginatedResponse<ServiceRequest>> {
   ...
   */
  validateServiceRequestData(data: ServiceRequestData): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
