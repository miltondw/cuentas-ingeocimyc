...
export class ApiquesService {
   ...
   */
  async getApiques(filters: ApiquesFilters): Promise<PaginatedResponse<Apique>> {
  ...
  async getApique(id: number): Promise<Apique> {
    const response = await api.get<ApiResponse<Apique>>(`${this.basePath}/${id}`);
  ...
  async createApique(apiqueData: CreateApiqueDto): Promise<Apique> {
    const response = await api.post<ApiResponse<Apique>>(this.basePath, apiqueData);
    if (!response.data.data) {
   ...
   */
  async updateApique(id: number, apiqueData: Partial<CreateApiqueDto>): Promise<Apique> {
    const response = await api.put<ApiResponse<Apique>>(`${this.basePath}/${id}`, apiqueData);
    if (!response.data.data) {
   ...
   */
  async updateApiqueStatus(id: number, status: 'collected' | 'analyzing' | 'completed' | 'reported'): Promise<Apique> {
    const response = await api.patch<ApiResponse<Apique>>(`${this.basePath}/${id}/status`, { status });
    if (!response.data.data) {
      throw new Error(response.data.error || 'Error al actualizar el estado del apique');
    }
    return response.data.data;
  }
   ...
   */
  async searchApiquesBySampleNumber(projectId: number, sampleNumber: string): Promise<PaginatedResponse<Apique>> {
    const filters: ApiquesFilters = {
      projectId,
      sampleNumber
    };
   ...
   */
  async getApiquesByStatus(projectId: number, status: 'collected' | 'analyzing' | 'completed' | 'reported'): Promise<PaginatedResponse<Apique>> {
    const filters: ApiquesFilters = {
      projectId,
      status
    };
   ...
   */
  async getApiquesByDateRange(projectId: number, startDate: string, endDate: string): Promise<PaginatedResponse<Apique>> {
    const filters: ApiquesFilters = {
      projectId,
   ...
   */
  async exportApiques(projectId: number, filters?: Omit<ApiquesFilters, 'projectId'>): Promise<Blob> {
    const params = new URLSearchParams();
